...
تُسلّم webhooks كرزون إشعارات HTTP فورية عند حدوث تغيير في حسابك في كرزون. سجّل نقطة نهاية HTTPS، واختر الأحداث التي تهمك، وترسل كرزون POST بحمولة JSON منظّمة كلما أُطلقت — مثلاً عند تعيين محادثة، أو تحديث عميل، أو اكتمال تدفق واتساب.
...
<Info>
  **MiniApp مقابل webhooks الحساب** — يغطي هذا الدليل webhooks **الصادرة** (كرزون تُخطر خادمك). لـ webhooks المزود **الواردة** المُعدّة داخل JSON لـ MiniApp، راجع [Webhooks في MiniApps](/ar/miniapps/guides/webhooks).
</Info>
...
<Info>
  استعلم `webhooksGetActions` لسرد كل حدث يمكن لحسابك في كرزون الاشتراك فيه. الكتالوج أدناه يعكس وحدات المنصة الحالية.
</Info>
